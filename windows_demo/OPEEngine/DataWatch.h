...
public:
    DataWatch(TArg init_data) : data(init_data), arg2p(init_data), allocater(CbHelper<DWMaxCnt>::get_allocater())
    {
        allocater.allocate_dw_stk<TArg, DWStkSz>(dw_stk); //argument passing here is gimped, check it
    }
    ...
    bool subscribe(TLambda&& lambda) {
        ...
        {
            CbWrapperDefined<TArg, TCb, CbWrprMaxSz> cb_wrpr(std::forward<TLambda>(lambda)); //create a temp wrapper object on stack to store callback

...
class CbAllocater
{
    private:
        ...
        {
            static_assert((DWStkSz + ... + 0) < OPEEconfigCB_POOL_SZ,
                    "CbPool size exceeded. Increase OPEEconfigCB_POOL_SZ or decrease DataWatch stack sizes.");
        }
    ...
    public:
        ...
        bool allocate_dw_stk(uint16_t& dw_stk)
        {
            ...
            if ((DWStkSz + allocater_ofs) < OPEEconfigCB_POOL_SZ)
            {
                if (dw_count < DWMaxCnt)
                {
                    ...
                    dw_stk_ptr_ofs[dw_count] =
                            allocater_ofs; // stack pointer starts at bottom of respective DWStk and grows with each cb subscribed
        ...
        bool allocate_cb(CbWrapperGeneric** subscribers, uint8_t& sub_count, const uint16_t dw_stk,
                CbWrapperDefined<TArg, TCb, CbWrprMaxSz>* cb_wrpr)
        {
            ...
            if (check_cb_max_sz_overflow<bytes2allocate, CbWrprMaxSz>())
            {
                ...
                if (dw_stk < dw_count) // was dw object correctly registered?
                {
                    ...
                    subscribers[sub_count++] = reinterpret_cast<CbWrapperGeneric*>(
                            cb_pool + dw_stk_ptr_ofs[dw_stk]); // save pointer to cb from cb_pool

...
const constexpr opee_size_t OPEEconfigCB_TASK_STK_SZ =
        4095UL; ///< Stack size for callback execution task in words (according to freeRTOS port, on a 32-bit system (256words)(4bytes) == 1kB)
const constexpr opee_size_t OPEEconfigCB_TASK_PRIO = 7UL; ///< Priority of callback task, modify as needed, preferably kept as high priority as possible.

const constexpr opee_uint8_t OPEEconfigCB_QUEUE_SZ = 15UL; ///< Size of callback queue, maximum amount of callbacks that can be waiting for execution at any given time.

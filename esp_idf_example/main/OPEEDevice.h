...
class OPEEDevice
{
    public:
        ...
        typedef struct led_t
        {
                ...
                DataWatch<bool, 32, 2, DWMaxCnt> state; // <type: bool (led on == true), total callback storage: 64b, max subs: 2>
                DataWatch<uint8_t, 32, 2, DWMaxCnt> pwm_percentage; // <type: uint8_t (0 = 0% duty cycle, 100 == 100% duty cycle), total callback storage: 64b, max subs: 3>
        } led_t;
        ...
        DataWatch<button_event_t, 64, 3, DWMaxCnt> usr_button; // type: custom (event generated by button), total callback storage: 48 bytes, max subscribers: 2
